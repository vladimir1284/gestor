...
from inventory.forms import PriceReferenceCreateForm
...
def export_inventory(request):
    ...
    return redirect(
        "https://docs.google.com/spreadsheets/d/1P5R4KUYcrxqCN3D-nDsDbBOJaPyDjARFlI_wPabdrT8/edit?usp=sharing"
    )
...
def minprice_update(request):
    if request.method == "POST":
        try:
            ...
            product.save()
            return JsonResponse({"status": "ok", "new_value": f"${product.min_price}"})
        except Exception as err:
            print(err)
            return JsonResponse({"status": "error", "msg": str(err)})
    return JsonResponse({})
...
def discountStockFIFO(product, product_quantity):
    ...
    stock_array = Stock.objects.filter(product=product).order_by("created_date")
...
def quantity_update(request):
    if request.method == "POST":
        try:
            ...
            if new_quantity < 0:
                return JsonResponse({"status": "error", "msg": "Negative quantity!!"})
            ...
            return JsonResponse({"status": "ok", "new_value": product.quantity})
        except Exception as err:
            print(err)
            return JsonResponse({"status": "error", "msg": str(err)})
    return JsonResponse({})
...
def minprice_product(request):
    ...
    for product in context["products"]:
        ...
        # Show references
        product.price_references = PriceReference.objects.filter(product=product)
...
def average(product: Product):
    transactions = ProductTransaction.objects.filter(
        order__type="sell", order__status="complete", product=product
    )
...
def detail_product(request, id):
    ...
    stocks = Stock.objects.filter(product=product, quantity__gt=0).order_by(
        "-created_date"
    )
    purchases = ProductTransaction.objects.filter(
        product=product, order__type="purchase", order__status="complete"
...
def update_price(request, id):
    ...
    # pass the object as instance in form
    form = PriceReferenceCreateForm(request.POST or None, instance=price)
