...
class CommonTransactionLayout(Layout):
    def __init__(self, *args, **kwargs):
        super().__init__(
            Div(
                Div(Field(PrependedText("price", "$")), css_class="col-6"),
                Div(
                    Div(
                        AppendedText("tax", "%"),
                    ),
            ...
            Div(
                Div(Field("quantity"), css_class="col-4"),
                Div(Div(Field("unit", css_class="form-select")), css_class="col-8"),
                css_class="row mb-3",
            ),
...
class KitTransactionCreateForm(forms.Form):
    ...
    def clean_quantity(self):
        ...
        for element in elements:
            required = convertUnit(
                element.unit, element.product.unit, element.quantity * quantity
            )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            ...
            Div(Div(Field("tax")), css_class="mb-3"),
...
class TransactionCreateForm(forms.ModelForm):
    class Meta:
        ...
        fields = ("price", "note", "quantity", "unit", "tax")
    ...
    def clean_price(self):
        ...
        if self.order.type == "sell":
            ...
            if unit != self.product.unit:
                error_msg += (
                    f"${price:.2f} for one {unit} implies ${product_cost:.2f} each {self.product.unit}. "
                    + average
                )

            if product_cost < self.limit:
                error_msg += (
                    f"The price cannot be lower than ${self.limit:.2f}." + average
                )
                raise ValidationError(error_msg)
        return price
...
class TransactionProviderCreateForm(TransactionCreateForm):
    ...
    class Meta:
        model = ProductTransaction
        fields = ("price", "note", "quantity", "unit", "tax", "associated")
            ...
            Div(
                Div(
                    Fieldset(
                        ...
                        Div(
                            Div(Field("image", css_class="form-select")),
                            css_class="mb-3",
                        ),
                        Div(Div(Field("description", rows="2")), css_class="mb-3"),
                        Div(
                            Div(Field("unit", css_class="form-select")),
                            css_class="mb-3",
                        ),
                        Div(
                        ...
                        Div(
                            Div(Field("type", css_class="form-select")),
                            css_class="mb-3",
                        ),
                        css_class="card-body",
                    ),
            ...
            Div(
                Div(
                    Fieldset(
                        ...
                        Div(Div(Field("active")), css_class="mb-3"),
                        Div(
                            Div(Field("quantity_min", css_class="form-select")),
                            css_class="mb-3",
                        ),
                        Div(
                            Div(Field(AppendedText("suggested_price", "%"))),
                            css_class="mb-3",
                        ),
                        Div(
                            Div(Field(PrependedText("min_price", "$"))),
                            css_class="mb-3",
                        ),
                        Div(
