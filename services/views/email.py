...
We also accept cash
Thanks""",
    "spanish": """Estimado {}, su vehículo en TOWITHOUSTON está listo!
Total a pagar: ${}
...
Check your invoice attached to this email.""",
    "spanish": """Gracias por elegir TOWITHOUSTON. Síganos en:
TikTok:
www.tiktok.com/@towithouston
Facebook:
www.facebook.com/towithouston
...
def mail_send_invoice(context, request, recipient_list):
    order = context["order"]
    ...
    if client:
        if order.status == "complete":
            ...
            body = PAYMENT_DICT[client.language].format(
                client.name, int(order.amount + order.tax), order.id
            )
        try:
            if settings.ENVIRONMENT == "production":
                ...
                if result:
                    with tempfile.NamedTemporaryFile(
                        suffix=".pdf", delete=False, prefix=f"invoice_towit_{order.id}_"
                    ) as output:
                        output.write(result)
                        output.flush()
                        with get_connection(
...
def sendMail(context, request, address, send_copy=False):
    generate_invoice_pdf(context, request)
