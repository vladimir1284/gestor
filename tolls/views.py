...
from .forms import TollCreateForm, TollUpdateForm
...
def create_toll(request, plate=None, contract=None):
    if not plate:
        context = {
            'plates': TrailerPlates.objects.all()
        }
    ...
    if request.method == "POST":
        form = TollCreateForm(request.POST, request.FILES, plate=_plate, contract=_contract)
        if form.is_valid():
            form.save()
            if next_id:
...
def list_toll(request, id=None):
    ...
    if not id:
        ...
        tolls = tolls.filter(created_date__year=currentYear,
...
def update_toll(request, id):
    ...
    if request.method == 'POST':
        form = TollUpdateForm(request.POST, request.FILES, instance=toll, plate=toll.plate)
        if form.is_valid():
            form.save()
            if next_id:
                return redirect(reverse('list-toll')+f"{next_id}")
